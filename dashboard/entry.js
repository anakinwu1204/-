...
function render(data){
 ...
 $('#entryHero').style.setProperty('--signal',state.color);$('#signal').style.setProperty('--signal',state.color);$('#signal span').textContent=state.label;$('#entryTitle').textContent=state.title;$('#entryCopy').textContent=`${state.copy} 固定獲利了結週期：進場後第 3 個交易日收盤。`;$('#riskScore').textContent=fmt(l.total,1);$('#riskScore').style.color=state.color;$('#asof').textContent=`依 ${l.date} 收盤資料判斷 · 下一交易日適用 · 持有 3 個交易日`;
 const items=[
 ...
 $('#shortCopy').textContent=shortWatch?'四項弱勢條件同時成立，但三日做空尚未通過樣本外驗證，只適合列入觀察。':`目前僅通過 ${shortPassed}/4 項弱勢條件，未形成做空觀察訊號。`;
