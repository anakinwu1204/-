...
function render(data){
 const l=data.latest,m=l.metrics,above=m.distance_ma60_pct>0,slope=m.ma60_slope_20d_pct>0,momentum=m.return_20d_pct>=-5&&m.return_20d_pct<=8,passed=[l.total<=45,above,slope,momentum].filter(Boolean).length,statusEmoji=passed===3?'💡 ':passed===0?'⚠️ ':'';
 const strong=l.total<=30,ready=l.total<=35&&above&&slope,pullback=l.total<=45&&above&&slope&&momentum;
 let state;
 ...
 const items=[
  ['總風險 ≤45',l.total<=45,`${fmt(l.total,1)} 分`,'回檔布局的基本風險門檻'],
