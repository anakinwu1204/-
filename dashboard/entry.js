...
function render(data){
 ...
 let state;
 if(strong)state={label:'適合分批做多',title:'低風險窗口已出現',copy:'總風險分數進入歷史回測中的強訊號區，可在下一交易日依資金規劃分批布局。',color:'#3ee0c2'};
 ...
 $('#entryHero').style.setProperty('--signal',state.color);$('#signal').style.setProperty('--signal',state.color);$('#signal span').textContent=state.label;$('#entryTitle').textContent=state.title;$('#entryCopy').textContent=state.copy;$('#riskScore').textContent=fmt(l.total,1);$('#riskScore').style.color=state.color;$('#asof').textContent=`依 ${l.date} 收盤資料判斷 · 下一交易日適用`;
 const items=[
