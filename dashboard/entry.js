...
function render(data){
 ...
 $('#entryHero').style.setProperty('--signal',state.color);$('#signal').style.setProperty('--signal',state.color);$('#signal span').textContent=state.label;$('#entryTitle').textContent=state.title;$('#entryCopy').textContent=`${state.copy} 固定獲利了結週期：進場後第 10 個交易日收盤。`;$('#riskScore').textContent=fmt(l.total,1);$('#riskScore').style.color=state.color;$('#asof').textContent=`依 ${l.date} 收盤資料判斷 · 下一交易日適用 · 持有 10 個交易日`;
 const items=[
