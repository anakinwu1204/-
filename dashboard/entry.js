...
function render(data){
 ...
 let state;
 if(strong)state={label:'適合分批做多',title:'低風險窗口已出現',copy:'總風險分數進入強訊號區；下一交易日分批進場，預計持有三個交易日後出場。',color:'#3ee0c2'};
 ...
 $('#entryHero').style.setProperty('--signal',state.color);$('#signal').style.setProperty('--signal',state.color);$('#signal span').textContent=statusEmoji+state.label;$('#entryTitle').textContent=state.title;$('#entryCopy').textContent=`${state.copy} 固定獲利了結週期：進場後第 3 個交易日收盤。`;$('#riskScore').textContent=fmt(l.total,1);$('#riskScore').style.color=state.color;$('#asof').textContent=`依 ${l.date} 收盤資料判斷 · 下一交易日適用 · 持有 3 個交易日`;
 const items=[
