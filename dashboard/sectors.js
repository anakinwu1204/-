...
function openStock(stock){const tone=stock.daily_return_pct>=0?'#3ee0c2':'#ff647c';$('#stockName').textContent=stock.name;$('#stockCode').textContent=stock.code;$('#stockPrice').textContent=`${fmt(stock.close,2)} 元`;$('#stockChange').textContent=`${stock.change>0?'+':''}${fmt(stock.change,2)}（${signed(stock.daily_return_pct)}）`;$('#stockChange').style.color=tone;$('#stockModal').hidden=false}
function closeStock(){$('#stockModal').hidden=true}
$('#stockClose').addEventListener('click',closeStock);$('#stockModal').addEventListener('click',e=>{if(e.target===$('#stockModal'))closeStock()});addEventListener('keydown',e=>{if(e.key==='Escape')closeStock()});
